...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
 Client-side routing grabs all needed resources upon initial data load, making it have near instant load times after initial load. Server-side only grabs what's currently needed, making it have quick initial load, but slightly delayed load after initial load.
